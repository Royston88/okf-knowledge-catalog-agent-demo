...
/**
 * Shared {@link gcp.ApiContext} for tests. It is built with the explicit
 * constructor so it carries fixed credentials and never triggers real gcloud
 * CLI calls.
 */
...
/**
 * In-memory mock of {@link gcp.CatalogClient} for tests. It backs all catalog
 * operations (entries, entry groups, entry types and aspect types) with local
 * collections so tests run without contacting the Dataplex Catalog API.
 */
export class CatalogClientMock extends gcp.CatalogClient {
  /** Entries returned by entry lookups, listing and mutation methods. */
  mockEntries: gcp.Entry[] = [];
  /** Entry groups keyed by their full resource name. */
  mockEntryGroups: Map<string, gcp.EntryGroup> = new Map();
  /** Entry types keyed by their full resource name. */
  mockEntryTypes: Map<string, gcp.EntryType> = new Map();
  /** Aspect types keyed by their full resource name. */
  mockAspectTypes: Map<string, gcp.AspectType> = new Map();
...
/**
 * In-memory mock of {@link bigquery.BigQueryClient} for tests, serving datasets
 * and tables from local maps instead of issuing real BigQuery API calls.
 */
export class BigQueryClientMock extends bigquery.BigQueryClient {
  /** Datasets keyed by their `projects/<p>/datasets/<d>` resource name. */
  mockDatasets: Map<string, bigquery.Dataset> = new Map();
  /** Tables keyed by their `.../datasets/<d>/tables/<t>` resource name. */
  mockTables: Map<string, bigquery.Table> = new Map();
...
/**
 * In-memory mock of {@link gcp.BigLakeClient} for tests, serving BigLake
 * tables from a local map instead of issuing real BigLake API calls.
 */
export class BigLakeClientMock extends gcp.BigLakeClient {
  /** BigLake tables keyed by their full resource name. */
  mockTables: Map<string, gcp.BigLakeTable> = new Map();
