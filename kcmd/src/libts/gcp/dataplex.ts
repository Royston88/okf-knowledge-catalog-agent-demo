...
/**
 * A Dataplex catalog entry group resource. Beyond the required `name`, the API
 * returns additional dynamically-typed fields that callers may read.
 */
...
/**
 * A Dataplex catalog entry type resource. Beyond the listed fields, the API may
 * return additional dynamically-typed properties.
 */
export interface EntryType {
  name: string;
  requiredAspects: Array<{type: string}>;
  [key: string]: unknown;
}

/**
 * A Dataplex catalog aspect type resource. Beyond the required `name`, the API
 * returns additional dynamically-typed fields that callers may read.
 */
...
/**
 * A single aspect attached to a catalog entry. `data` holds the aspect's
 * dynamically-typed payload as returned by the API.
 */
...
/**
 * A Dataplex catalog entry, representing a single cataloged resource along with
 * its source metadata and attached aspects.
 */
...
/**
 * A reference from an entry link to one of its endpoint entries, identifying the
 * referenced entry by name and its role/type in the link.
 */
...
/**
 * A directional link between catalog entries (e.g. a relationship), described by
 * its type, the entries it references, and any attached aspects.
 */
...
/**
 * Response payload of the `lookupEntryLinks` API, holding the matched entry
 * links and an optional pagination token.
 */
...
/**
 * A Dataplex business glossary resource, a container for glossary terms and
 * categories.
 */
...
/**
 * A single term within a Dataplex business glossary, nested under its parent
 * glossary or category.
 */
...
/**
 * A category within a Dataplex business glossary, used to group glossary terms
 * under its parent glossary or category.
 */
...
/**
 * Client for the Dataplex Knowledge Catalog API, providing CRUD and listing
 * operations for entries, entry links, glossaries, terms and categories. It
 * normalizes resource names to use project ids consistently.
 */
export class CatalogClient extends api.ApiClient {
  ...
  async modifyEntry(
    ...
    const body: {entry: Entry; updateMask?: string; aspectKeys?: string[]} = {
      'entry': entry,
  ...
  async createEntryLink(
    ...
    const params: Record<string, string> = {'entryLinkId': entryLinkId};
  ...
  async createEntry(
    ...
    const params: Record<string, string> = {'entryId': entryId};
  ...
  async createEntryGroup(
    ...
    const params: Record<string, string> = {'entryGroupId': entryGroupId};
  ...
  async createGlossary(
    ...
    const parent = catalogContainer(project, location);
    const resourceName = `${parent}/glossaries`;
    const params: Record<string, string> = {'glossaryId': glossaryId};
  ...
  async createGlossaryTerm(
    ...
    const parent = `${catalogContainer(project, location)}/glossaries/${glossary}`;
    const resourceName = `${parent}/terms`;
    const params: Record<string, string> = {'termId': termId};
...
/**
 * Constructs canonical names for catalog container resources, identified by
 * project, location and optionally, depending on use-case, the entry group.
 */
...
/**
 * Converts a `project.location.type` reference into a canonical resource name of
 * the form `projects/{project}/locations/{location}/{type}Types/{type}`.
 */
...
/**
 * Converts a canonical resource name of the form
 * `projects/{project}/locations/{location}/{type}Types/{type}` back into a
 * `project.location.type` reference.
 */
...
/**
 * Wraps a bare resource name into a Dataplex proxy entry name under the
 * `@dataplex` entry group, leaving names that are already wrapped or not
...
/**
 * Reverses {@link wrapAsProxyEntry}, extracting the underlying resource name
 * from a Dataplex `@dataplex` proxy entry name, or returning the input
 * unchanged if it is not a proxy entry name.
 */
...
/**
 * Normalizes the project portion of an entry link's name, type and referenced
 * entry names so they consistently use project ids rather than project numbers.
 */
