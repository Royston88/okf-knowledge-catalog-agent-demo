...
/**
 * Resolved reference catalog: the source for the reference scope together with
 * its parsed snapshot configuration.
 */
...
/**
 * On-disk YAML shape produced by {@link CatalogManifest.save}. Mirrors the
 * fields accepted by the manifest schema; optional fields are omitted when
 * undefined.
 */
...
/**
 * In-memory representation of a catalog manifest: the catalog source plus its
 * snapshot, publishing, reference and alias configuration. Provides factory
 * helpers to construct manifests for the supported source types and to
 * load/save the manifest to a YAML file on disk.
 */
export class CatalogManifest implements CatalogManifestLike {
  readonly source: CatalogSource;
  ...
  static parseAlias(
    ...
    const aliasMap: ResourceAlias = new ResourceAlias();
    if (aliasList) {
      for (const [alias, resource] of Object.entries(aliasList) as Array<
    ...
    if (snapshot) {
      ...
      if (snapshot.aspects) {
        for (const aspectType of snapshot.aspects) {
          const formalName: string = aliasMap
  ...
  static async parseReference(
    ...
    if (reference) {
      const referenceSource: CatalogSource = await CatalogManifest.parseScope(
        reference.scope,
        ctx,
      );
      const referenceSnapshot: SnapshotConfig | undefined =
        CatalogManifest.parseSnapshot(reference.snapshot, aliasMap);
  ...
  static async load(
    ...
    const rawScope = result.data.scope;
    const scope = Array.isArray(rawScope)
      ? rawScope.map(s => expandEnvVars(s))
      : expandEnvVars(rawScope);

    const source: CatalogSource = await CatalogManifest.parseScope(
      scope,
      ctx,
    );

    const aliasMap: ResourceAlias = CatalogManifest.parseAlias(
      result.data.resourceAlias,
    );

    const snapshot: SnapshotConfig | undefined = CatalogManifest.parseSnapshot(
      result.data.snapshot,
      aliasMap,
    );

    const publishing: PublishingConfig | undefined =
      CatalogManifest.parsePublishingConfig(
        snapshot,
        result.data.publishing,
        aliasMap,
      );

    const reference: ReferenceManifest | undefined =
      await CatalogManifest.parseReference(
