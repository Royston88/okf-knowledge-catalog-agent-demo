...
import {CatalogSource} from '../core/source';
import * as gcp from '../gcp';
import * as dataplex from '../gcp/dataplex';

/**
 * Union of the Dataplex glossary resource types that this source emits and
 * maps to/from local files: a glossary, one of its categories, or one of its
 * terms. All three share `name` and `displayName`, which are the fields used
 * for local-path computation.
 */
...
/**
 * Catalog metadata source backed by the Dataplex Business Glossary. It
 * enumerates glossaries (optionally restricted to a specific set) together
 * with their categories and terms, and translates between Dataplex REST
 * resource names and the local file layout used by kcmd.
 */
export class GlossarySource implements CatalogSource {
  ...
  async *entries(
    ctx: gcp.ApiContext,
  ): AsyncGenerator<GlossaryResource, void, unknown> {
  ...
  localName(resource: GlossaryResource, isReference?: boolean): string {
