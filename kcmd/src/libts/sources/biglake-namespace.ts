...
/**
 * A {@link CatalogSource} that exposes the tables of a single BigLake
 * Metastore namespace as catalog metadata entries.
 *
 * The source is configured with a fully qualified namespace name in the form
 * `<projectId>.<catalogId>.<namespaceId>`. It lists the Iceberg tables in that
 * namespace via the BigLake API, looks up the corresponding Dataplex catalog
 * entries, and provides mapping between local file names and BigLake service
 * resource names.
 */
