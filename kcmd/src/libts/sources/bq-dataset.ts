...
/**
 * A {@link CatalogSource} backed by one or more BigQuery datasets.
 *
 * Iterates over the configured datasets to yield their Dataplex catalog
 * entries (the dataset entry plus each table entry) and provides mappings
 * between local catalog paths (e.g. `bigquery/<project>/<dataset>/<table>`)
 * and the fully-qualified Dataplex service entry names.
 */
