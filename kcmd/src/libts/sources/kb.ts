...
/**
 * A `CatalogSource` backed by a Dataplex Wiki EntryGroup.
 *
 * Treats a Dataplex EntryGroup as a knowledge-base metadata source, mapping
 * its entries to and from local catalog paths of the form
 * `<namespace>/<project>/<location>/<entryId>`. It enumerates entries from the
 * EntryGroup and translates between local names (used in the document layout)
 * and fully-qualified service entry names.
 */
