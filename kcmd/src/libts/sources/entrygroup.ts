...
/**
 * A {@link CatalogSource} backed by a single Dataplex EntryGroup.
 *
 * Enumerates the entries of a specific EntryGroup (identified by a dotted
 * `project.location.entryGroup` name) and maps between their fully-qualified
 * service resource names and the local catalog layout
 * (`<namespace>/<project>/<location>/<entryId>`). The `@`-prefixed third name
 * segment marks a namespace whose entries were ingested rather than authored
 * locally.
 */
