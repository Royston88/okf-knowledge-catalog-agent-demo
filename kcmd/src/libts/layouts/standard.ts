...
import {CatalogManifestLike} from '../core/manifest';
import * as md from '../core/metadata';

/**
 * Standard on-disk catalog layout that stores each entry as a YAML file within
 * the catalog directory, optionally augmented by Markdown sidecar files for
 * Markdown-bearing aspects (e.g. `overview`). Supports a two-layer model where a
 * `.ref.yaml` reference layer is merged with a local `.yaml` layer, with local
...
export class StandardLayout implements CatalogLayout {
  ...
  private async _loadLayer(entryPath: string): Promise<md.Entry> {
    ...
    try {
      ...
      for (const sidecarFile of sidecarFiles) {
        ...
        if (this.manifest) {
          const snapshotAspects = this.manifest.snapshotConfig?.aspects || [];
          const publishingAspects =
            this.manifest.publishingConfig?.aspects || [];
  ...
  async saveEntry(name: string, entry: md.Entry): Promise<void> {
    ...
    if (entryClone.aspects) {
      ...
      for (const key of Object.keys(entryClone.aspects)) {
...
function toSidecarMarkdown(aspectData: md.Aspect): string {
  const cloned = JSON.parse(JSON.stringify(aspectData)) as Record<
    string,
    unknown
  >;
  const body = typeof cloned['content'] === 'string' ? cloned['content'] : '';
...
function isMarkdownAspect(key: string, data: md.Aspect): boolean {
  if (
    key === 'overview' ||
