...
import type {CatalogManifestLike} from '../core/manifest';
...
/**
 * Catalog layout that stores Dataplex entries as an OKF (Open Knowledge Format)
 * bundle: a directory of per-concept markdown files with YAML frontmatter, plus
 * reserved `index.md` directory listings. The full `md.Entry` is stashed under a
...
export class OkfLayout implements CatalogLayout {
  ...
  constructor(catalogPath: string, manifest?: CatalogManifestLike) {
  ...
  async init(): Promise<void> {
    ...
    const synthIndexEntries = !this.manifest?.source?.ingestedEntries;
  ...
  private async _synthIndexEntry(
    ...
    // are meaningless in the Google Cloud Console UI (they resolve to broken
    // URLs), so the KC index ENTRY's overview is de-linked to plain text.
    // The on-disk OKF bundle keeps its links.
    ...
    if (parentId && this.manifest) {
      entry.resource.parent = this.manifest.source.serviceName(parentId);
    }
    return entry;
  }
...
/**
 * Parses an OKF concept file into a Dataplex entry and its markdown body.
 * Prefers the faithful `x-kcmd` stash when present and otherwise reconstructs
 * what it can from the OKF spec frontmatter fields (a lossy path that keeps
 * hand-authored / external OKF files loadable). Returns a `null` entry for files
 * that carry no frontmatter or no real entry (e.g. a root index.md).
 */
...
/**
 * Serializes a Dataplex entry plus its markdown body into an OKF concept file:
 * OKF-spec frontmatter fields derived from the entry, the full entry stashed
 * under the hidden `x-kcmd` key for lossless round-tripping, then the body.
 */
...
// to plain text — these resolve to broken URLs in the Google Cloud Console UI.
