...
/**
 * Any catalog resource in the Dataplex service representation that can be stored
 * into, or fetched from, a local catalog snapshot. Entries are distinguished
 * from glossary resources by the presence of the `entryType` field.
 */
export type DataplexResource =
  | dataplex.Entry
  | dataplex.Glossary
...
/**
 * A single field within a `schema` aspect's `fields` array. Schema fields may
 * carry their own entry links (e.g. glossary-term or join links), keyed by link
 * type alias.
 */
interface SchemaField {
  name?: string;
  links?: Record<string, md.EntryLink[]>;
}

/**
 * An in-memory view over a locally checked-out catalog directory. It loads the
 * catalog manifest and the relevant Dataplex entry/aspect type definitions,
 * then mediates reads and writes of entries through the configured on-disk
 * layout, translating between the Dataplex service representation and the local
 * metadata representation during sync (pull/push) operations.
 */
export class CatalogSnapshot {
  /** The parsed catalog manifest describing this snapshot's source and config. */
  readonly manifest: CatalogManifest;
  /** Absolute filesystem path to the root of the local catalog checkout. */
  ...
  /**
   * Loads a catalog snapshot from a local checkout directory, parsing its
   * manifest and resolving the entry/aspect type definitions it references.
   */
  ...
  /** The entry types known to this snapshot, keyed by resource and type name. */
  get entryTypes(): Map<string, dataplex.EntryType> {
    return this._entryTypes;
  }

  /** The aspect types known to this snapshot, keyed by resource and type name. */
  get aspectTypes(): Map<string, dataplex.AspectType> {
    return this._aspectTypes;
  }

  /** The reference snapshot's entry types, keyed by resource and type name. */
  get referenceEntryTypes(): Map<string, dataplex.EntryType> {
    return this._referenceEntryTypes;
  }

  /** The reference snapshot's aspect types, keyed by resource and type name. */
  ...
  async _storeResource(
    ...
    if ((resource as dataplex.Entry).entryType) {
      localResource = await toLocalEntry(
        resource as dataplex.Entry,
        internalName,
      ...
      localResource = toLocalGlossaryTerm(
        resource as dataplex.GlossaryTerm,
        internalName,
      );
    } else if (resource.name.includes('/categories/')) {
      localResource = toLocalGlossaryCategory(
        resource as dataplex.GlossaryCategory,
        internalName,
      );
    } else {
      localResource = toLocalGlossary(
        resource as dataplex.Glossary,
        internalName,
      );
    }
  ...
  async _fetchResource(name: string): Promise<DataplexResource | undefined> {
...
async function toLocalEntry(
  ...
  if (entry.aspects) {
    for (const key of Object.keys(entry.aspects)) {
          ...
          if (pathParts[0]?.toLowerCase() === 'schema' && pathParts[1]) {
            ...
            if (schemaAspect && Array.isArray(schemaAspect['fields'])) {
              const field = schemaAspect['fields'].find(
                (f: SchemaField) => f.name === pathParts[1],
              );
              if (field) {
                if (!field.links) {
                  field.links = {};
