...
/**
 * Outcome of a sync operation (`pull`, `reference`, or `push`). `success`
 * reports whether the operation completed without error, and `details`
 * carries a human-readable failure message when it did not.
 */
...
/**
 * Outcome of validating the local snapshot against the catalog service.
 * `valid` is true when the snapshot passes validation.
 */
...
/**
 * Outcome of a status check comparing the local snapshot to the catalog
 * service. `modified` is true when local metadata differs from the remote
 * state.
 */
...
/**
 * Drives catalog synchronization between a local {@link CatalogSnapshot} and
 * the Dataplex catalog service. Provides `pull` (fetch remote metadata into
 * the snapshot), `reference` (fetch baseline metadata into `.ref.yaml`), and
 * `push` (publish local metadata, including entries, entry links, and
 * glossary updates) operations.
 */
export class CatalogSync {
  ...
  async pull(options?: {dryRun?: boolean}): Promise<SyncResult> {
    try {
      ...
      for await (const resource of resources) {
        ...
        if ('entryType' in resource && resource.entryType) {
          if (
  ...
  async push(options?: {
    ...
    // EntryLink creation validates that BOTH endpoints already exist. Because
    // links are reconciled inside the per-entry loop below, a link whose TARGET
    // entry sorts AFTER its source in this same push (e.g. posts_answers ->
    // posts_questions) would 404 and abort, leaving most entries uncreated. So
    // we DEFER any link that fails because its target is not yet created and
    // RETRY the deferred links after the loop, once every entry exists.
    const deferredLinks: Array<{
      project: string;
      location: string;
      entryGroup: string;
      localLink: gcp.EntryLink;
    }> = [];
        ...
        if (exist.status !== 200 || !exist.result) {
          if (options?.dryRun) {
            ...
            if (!this._verifiedEntryGroups.has(groupKey)) {
              ...
              if (groupRes.status !== 200 || !groupRes.result) {
                ...
                // NOTE: entry-group creation is eventually consistent ACROSS
                // services — right after createEntryGroup the catalog API reports
                // the EG exists, but the entries API can still return
                // ENTRY_GROUP_NOT_FOUND for a short window. We therefore retry the
                // createEntry below on that error (getEntryGroup lies here).
              }
              this._verifiedEntryGroups.add(groupKey);
            }

            let createEntryRes = await this._catalog.createEntry(
              project,
              location,
              entryGroup,
              entryId,
            ...
            // A just-created entry group is eventually consistent across services:
            // the entries API can return ENTRY_GROUP_NOT_FOUND for a short window
            // after the EG was created. Retry the create with backoff while the
            // failure is specifically "entry group not found" (~24s cap); other
            // errors fail fast.
            for (
              let attempt = 0;
              attempt < 12 &&
              (createEntryRes.status !== 200 || !createEntryRes.result) &&
              /ENTRY_GROUP_NOT_FOUND|entry group .*does not exist/i.test(
                String(createEntryRes.message || ''),
              );
              attempt++
            ) {
              await new Promise((resolve) => setTimeout(resolve, 2000));
              createEntryRes = await this._catalog.createEntry(
                project,
                location,
                entryGroup,
                entryId,
          ...
          if (updateMask.length) {
            if (options?.dryRun) {
              ...
              console.log(`[DEBUG] Modifying entry: ${entry.name}`);
              console.log(`[DEBUG] UpdateMask: ${JSON.stringify(updateMask)}`);
              console.log(`[DEBUG] AspectKeys: ${JSON.stringify(aspectKeys)}`);
              console.log(`[DEBUG] Entry payload: ${JSON.stringify(entry)}`);
        ...
        if (existingLinksRes.status === 200 && existingLinksRes.result) {
          ...
          for (const [key, localLink] of localByKey) {
            ...
            if (options?.dryRun) {
              ...
              if (res.status !== 200) {
                // The TARGET entry may not exist YET if it sorts later in this
                // same push. Defer and retry after all entries are created;
                // fail fast on any other error.
                if (
                  /ENTRY_NOT_FOUND|does not exist/i.test(
                    String(res.message || ''),
                  )
                ) {
                  console.log(
                    `[DEBUG] Deferring EntryLink (target not created yet): ${res.message || ''}`,
                  );
                  deferredLinks.push({
                    project,
                    location,
    ...
    // Retry EntryLinks that were deferred because their TARGET entry sorted
    ...
    if (deferredLinks.length && !options?.dryRun) {
      ...
      let skipped = 0;
      for (const {project, location, entryGroup, localLink} of deferredLinks) {
        const linkId = 'link-' + Math.random().toString(36).substring(2, 12);
        const res = await this._catalog.createEntryLink(
          project,
          location,
          entryGroup,
        ...
        console.log(
          `[DEBUG] RETRY CREATE EntryLink result: ${res.status} ${res.message || ''}`,
        );
        if (res.status !== 200) {
          // A link whose SOURCE is one of our entries (now created) resolves on
          // retry. A still-missing TARGET means it points at an EXTERNAL,
          // read-only entry that isn't in this catalog (e.g. an `@bigquery`
          // table not auto-cataloged in Dataplex). That cross-reference can
          // never be created here, so warn and skip rather than aborting the
          // whole push over an unmanaged external target; fail fast otherwise.
          if (
            /ENTRY_NOT_FOUND|does not exist/i.test(String(res.message || ''))
          ) {
            const target = localLink.entryReferences.find(
              (r) => r.type === 'TARGET',
            )?.name;
            console.warn(
              `[WARN] Skipping EntryLink to missing external entry ${target}: ${res.message || ''}`,
            );
            skipped++;
          } else {
            throw new Error(
              `Failed to create EntryLink after retry: ${res.message || res.status}`,
            );
          }
        }
      }
      if (skipped) {
        console.log(
          `Skipped ${skipped} EntryLink(s) targeting external entries not present in the catalog.`,
        );
      }
    }
