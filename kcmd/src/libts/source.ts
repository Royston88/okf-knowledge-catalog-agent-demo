// Catalog metadata source registry/factory. The source CONTRACTS
// (CatalogSource / Sources / SourceResource) live in ./core/source and are
// re-exported here for back-compat; this module wires the concrete source
// implementations under ./sources/ via createSource().
//

import {CatalogSource, Sources} from './core/source';
...
/**
 * Factory that resolves a source `name` against the Catalog/BigQuery services
 * and constructs the {@link CatalogSource} implementation matching the given
 * `type`. Throws if the type is unknown or the underlying resource cannot be
 * located.
 */
