<!--
  This is the repository's original README from `main`, preserved verbatim.

  The `v6z-okf-projector` branch took over the root README to document the
  projector, which would otherwise have deleted the only setup guide the demo,
  the ADK agent and the sync service have. It is moved rather than rewritten:
  nothing below this comment has been edited, including the parts the projector
  branch disagrees with. Its commands still run from the repository root.

  Where the projector's findings bear on something here, the root README says
  so — see "The one thing that does overlap" for `kcmd-sync-service`.
-->
