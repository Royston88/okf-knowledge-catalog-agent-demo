...
| Glossary `okf-metric-probe` (us) | **probe, still live** — term `avg-monthly-balance` attached to BOTH `balance_snapshots.balance` and `accounts.balance` (demonstrating one-term-many-columns). `python okf-review/probe_glossary.py --teardown` |
