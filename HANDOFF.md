...
| Glossary `okf-metric-probe` (us) | **probe, still live** — term `avg-monthly-balance` attached to `balance_snapshots.balance` only (the `accounts.balance` attachment was semantically wrong and was removed). `python okf-review/probe_glossary.py --teardown` |
